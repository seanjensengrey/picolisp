...
function csMouseDn(cvs, event) {
   cvs.csDn = true;
   cvs.csDnX = event.layerX - cvs.offsetLeft;
   cvs.csDnY = event.layerY - cvs.offsetTop;
   cvs.csMv = false;
   return false;
}
...
function csMouseMv(cvs, event) {
   ...
   if (drawCanvas(cvs.id, -1, cvs.csMv? -1 : 0, cvs.csDnX, cvs.csDnY,
         event.layerX - cvs.offsetLeft, event.layerY - cvs.offsetTop ) )
      return true;
   cvs.csMv = true;
   return false;
}
...
function renderCanvas(cvs, lst) {
   ...
   for (i = 0; i < lst.length; ++i) {
      ...
      case 19:  // (csDrawDots DX DY Lst)
         ...
         break;
      case 20:  // (csDrawImage Img X Y)
         var img = new Image();
         img.src = cmd[1];
         (function (img, cmd) {img.onload = function () {ctx.drawImage(img, cmd[2], cmd[3])}})(img, cmd);
         break;
      ...
      case 25:  // (csRestore)
         ...
         break;
      /*** Variables ***/
      case 26:  // (csFillStyle V)
         ctx.fillStyle = cmd[1];
         break;
      case 27:  // (csStrokeStyle V)
         ctx.strokeStyle = cmd[1];
         break;
      case 28:  // (csGlobalAlpha V)
         ctx.globalAlpha = cmd[1];
         break;
      case 29:  // (csLineWidth V)
         ctx.lineWidth = cmd[1];
         break;
      case 30:  // (csLineCap V)
         ctx.lineCap = cmd[1];
         break;
      case 31:  // (csLineJoin V)
         ctx.lineJoin = cmd[1];
         break;
      case 32:  // (csMiterLimit V)
         ctx.miterLimit = cmd[1];
         break;
      case 33:  // (csGlobalCompositeOperation V) )
