...
function csMouseMv(cvs, event) {
   ...
   if (!cvs.csDn)
      return true;
   if (drawCanvas(cvs.id, CvsDly, cvs.csMv? -1 : 0, cvs.csDnX, cvs.csDnY,
...
function csMouseOut(cvs) {
   ...
   return drawCanvas(cvs.id, CvsDly);
}
...
function csMouseUp(cvs) {
   ...
   if (cvs.clicked) {
      clearTimeout(cvs.clicked);
      cvs.clicked = false;
      return drawCanvas(cvs.id, CvsDly, 2, cvs.csDnX, cvs.csDnY);
   }
   if (cvs.csMv)
      return drawCanvas(cvs.id, CvsDly);
   cvs.clicked = setTimeout(
      function() {
         cvs.clicked = false;
         drawCanvas(cvs.id, CvsDly, 1, cvs.csDnX, cvs.csDnY);
      },
      200 );
   return false;
}
...
function drawCanvas(id, dly) {
   if (CvsReq.readyState > 0 && CvsReq.readyState < 4)
      return true;
   ...
   for (var i = 2; i < len; ++i)
      if (typeof arguments[i] === "number")
         ...
         url += "&" + arguments[i];
   try {CvsReq.open("POST", url);}
   catch (e) {return true;}
   CvsReq.responseType = "arraybuffer";
   CvsReq.onload = function() {
      var ele = document.getElementById(id);

      renderCanvas(ele, plio(new Uint8Array(CvsReq.response)));
      if (ele.post) {
         ele.post = false;
